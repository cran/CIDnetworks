CIDnetworks: How It Works, And What It Fits
========================================================

A.C. Thomas, December 13, 2013
(recent changes: added pictures of each type of plot.)
...
Suppose we have a Gilbert model for a network: every edge has the same probability of existing. For $\mu = -1,$ this corresponds to $P(Y_{ij}=1) = \Phi(-1) = 0.159$. Below we have the latent strengths generated with this $\mu$, and beside it, the binary network created as a result. 

```{r fig.width=12, fig.height=6, echo=FALSE}
net1 <- CID(n.nodes=20, intercept=-1, generate=TRUE)
par(mfrow=c(1,2))
net1$plot.network(net1$int.outcome, main="Values of Latent Strengths")
net1$plot.network(main="Observed Network Edges")
```
